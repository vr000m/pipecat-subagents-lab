...
- **Recorded for Phase 3 (do not misread doubles conformance):** `FakeCoordinator` defaults `config` to a real `Config()` while `CoordinatorDefaults.config` is `None`, so post-migration most doubles supply a Config and `coordinator_view`'s `getattr(coordinator, "config", None)` fallback branch lost test coverage (the `RoutedCoordinator` workaround at test_pipeline.py:52-64 resets `config = None` where tests depend on absence). Phase 3's Requirement 1 re-litigation must NOT treat "doubles now conform" as evidence the getattr-fallbacks are dead — the fallback path is exercised less, not proven unnecessary. `BareCoordinator` remains the deliberate fallback-path test.
