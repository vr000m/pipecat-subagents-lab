...
- Render transcript with compact server-authored timestamps, current routing state, persistent worker list, per-result worker identity, structured answer, and sources.
...
- Keep long inline worker-result diagnostics from dominating the transcript: show at most a five-line preview and a disclosure caret when the answer, delivery status, and source list exceed that limit; expansion exposes the complete result without truncating canonical state.
...
- Browser render tests prove user and assistant turns expose server-authored timestamps, long live result diagnostics receive a five-line disclosure preview, and short diagnostics remain unadorned.
