...
- Start one process-lifetime `WorkerRunner` for persistent `BaseWorker` bus participation. Pipecat 1.6.0 has no connection-worker removal API, so each promoted browser connection runs as a directly owned `PipelineWorker.run()` lifecycle task containing the framework `BusBridgeProcessor`; replacement cancels and awaits that task without adding it to the durable runner registry or rebuilding persistent worker contexts.
...
- **Process-lifetime vs connection-lifetime boundary:** persistent workers, their contexts, canonical results, pending-dialogue state, and the session sequence counter outlive any single connection. Each promoted browser connection receives one directly owned connection-lifetime `PipelineWorker.run()` task containing Small WebRTC, STT/TTS, `BusBridgeProcessor`, canonical-result adapter, and RTVI state publisher; replacement cancels and awaits that task, then builds a fresh one without adding it to the durable runner or rebuilding persistent worker contexts.
...
- Spoken and UI projections share a canonical result ID and citation set. The provider is instructed to keep the concise speech projection grounded in the display answer and sources; v0.1 runtime schema validation does not prove or evaluate semantic equivalence.
...
- **A greeting incorrectly bootstrapped a persistent web-search worker and later turns mutated its topic:** classify greetings and casual conversation as direct before worker selection, require existing-worker metadata to be copied verbatim from the immutable catalogue, retain fail-closed validation, and add the opt-in live `--routing-regression` smoke sequence.
...
starts a directly owned Pipecat `SmallWebRTCTransport`/`PipelineWorker` task,
then cancels that task when the connection closes. The process-lifetime runner
owns only durable application workers. Daily transport remains a deployment adapter to add only if this lab is
